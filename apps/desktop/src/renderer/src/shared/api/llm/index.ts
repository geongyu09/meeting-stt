import type { SetClaudeApiKeyRequest, SetLlmProviderRequest } from '@shared/ipc'
 ...
 * @returns 공급자, 로컬 모델 준비 여부, 키 유무, `claude` 실행 파일 경로·버전
 * @example
 ...
 * @param provider - 'local' | 'claude-api' | 'claude-cli'
 * @returns 갱신된 상태
 * @example
...
/**
 * @description Claude API 키를 저장하거나 지웁니다. main이 암호화해 저장하며 키를 되돌려주지 않습니다.
 * @param apiKey - 저장할 키. `null`이면 저장된 키를 지웁니다
 * @returns 갱신된 상태
 * @example
 * const status = await setClaudeApiKeyApi({ apiKey: 'sk-ant-…' })
 */
export const setClaudeApiKeyApi = async ({ apiKey }: SetClaudeApiKeyRequest) =>
  invokeLlm({
    call: () => window.api.llm.setClaudeApiKey({ apiKey }),
