 ...
 * @returns 저장된 설정
 * @example
 * const settings = await updateSettingsApi({ isAudioKept: true, isUpdateCheckEnabled: false })
 */
export const updateSettingsApi = async ({
  isAudioKept,
  isUpdateCheckEnabled
}: UpdateSettingsRequest) => window.api.settings.update({ isAudioKept, isUpdateCheckEnabled })
