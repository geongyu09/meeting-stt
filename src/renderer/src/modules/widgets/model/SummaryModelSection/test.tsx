...
describe('SummaryModelSection', () => {
  it('요약 모델이 없으면 용량과 함께 다운로드 버튼을 보여준다', async () => {
    ...
    expect(screen.getByRole('button', { name: '요약 모델 다운로드 (2.5GB)' })).toBeTruthy()
  ...
  it('버튼을 누르면 진행률을 보여주고 끝나면 설치됨으로 바뀐다', async () => {
    ...
    await user.click(screen.getByRole('button', { name: /요약 모델 다운로드/ }))
  ...
  it('실패하면 안내를 보여주고 다시 받을 수 있다', async () => {
    ...
    expect(screen.getByRole('button', { name: /요약 모델 다운로드/ })).toBeTruthy()
  })
})
